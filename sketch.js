...
rect_density = 1.6*Math.pow(10,-13)*hole_add 
if (isOn ==false) {
//right up 
rect((550)*s_x,(10+385/2+96.25)*s_y,145*s_x, -count_pn_num*1.5*10*s_y)
//left down
rect((10+100+70+change_square + (940-change_length-70)/2)*s_x,(10+385/2+96.25)*s_y,-145*s_x, count_pn_num*1.5*10*s_y)
...
//E- field 
if (isOn==true){
  triangle((10+100+70+change_square + (940-change_length-70)/2-145)*s_x, (10+385/2+96.25)*s_y, (10+100+70+change_square + (940-change_length-70)/2+145)*s_x, (10+385/2+96.25)*s_y, (10+100+70+change_square + (940-change_length-70)/2)*s_x, (10+385/2+96.25+  2*count_pn_num*10)*s_y)
...
let V_built = 0.052*Math.log(hole_add/Math.pow(10,10))
let X_n = 5811*Math.pow(Math.log(hole_add/Math.pow(10,10))/(Math.pow(10,6)*hole_add),1/2)*Math.pow(10,6)
// let array_band = []
for (var i = 0; i<20; i++) {
  ...
  stroke(125, 241, 148,100);
  noFill()
  // rect((150+(800)/20*i)*s_x,(10)*s_y,(800)/20*(i)*s_x,(770/4)*s_y);

  if ((150+(800)/20*i)*s_x <(550-145)*s_x ){
    array_band[i] = 0

  } 
  if ( (550-145)*s_x <= (150+(800)/20*i)*s_x && (150+(800)/20*i)*s_x <(550)*s_x ){
    array_band[i] = -7.76*Math.pow(10,-16)*hole_add*Math.pow((145+(150+(800)/20*i-550)*s_x),2)*Math.pow(10,-2)
    //array_band[i] = -7.76*Math.pow(10,-16)*hole_add*Math.pow((145+x),2)
  }
   if ( (550)*s_x <= (150+(800)/20*i)*s_x && (150+(800)/20*i)*s_x <(550+145)*s_x ){
    array_band[i] = V_built*100-7.76*Math.pow(10,-16)*hole_add*Math.pow((145-(150+(800)/20*i-550)*s_x),2)*Math.pow(10,-2)
  } 
   if ( (550+145)*s_x <= (150+(800)/20*i)*s_x  ){
    array_band[i] = V_built*100
  }
